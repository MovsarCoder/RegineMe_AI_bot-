...
from telegram_bot.keyboard.keyboards import get_accept_cancel_buttons, admin_keyboard

router = Router()


@router.callback_query(F.data == 'show_requests_cooperation')
...
async def show_requests_cooperation_2(callback: CallbackQuery):
    ...
    if requests:
        ...
        await callback.message.edit_text("<b>❌ Пока что нет новых заявок на сотрудничество.</b>", reply_markup=make_row_inline_keyboards(admin_keyboard))
